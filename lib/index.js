    ...
    .then(function(tables){
      ...
      async.each(tables, function(table, _callback){
        ...
        async.each(tableNames, function(table, _callback){
          ...
          //TYPESCRIPT: 'import * as Sequelize from 'sequelize''
          typeScriptInterface[table] = "import * as Sequelize from 'sequelize'\n\n";
          //TYPESCRIPT: 'export interface I[table name] extends Sequelize.Instance<[table name], {}> {'
          typeScriptInterface[table] += "export interface I" + table + " extends Sequelize.Instance<I" + table + ", {}> {\n";
              ...
              fieldAttr.forEach(function(attr, x){
                ...
                if (attr === "primaryKey") {
                  ...
                  if (Sequelize.Utils._.isString(defaultVal)) {
                    if(val_text === "(getdate())"){
                     val_text = "function(){ return new Date(); }";
                    }else{
                     val_text = "'" + val_text + "'"
                    }
                  }
          ...
          text[table] += spaces + spaces + spaces + "getPrimaryKeyName: function() { return '" + firstKey + "' ; }\n";
          text[table] += spaces + spaces + "}\n";
          text[table] += spaces + "}";
